 ...
 * Complexity:  O(n)
 *
 * Description:    create two sublists divided by the pivot value in the middle, return index of pivot value
 *		MAXELTS.
 */

static int partition(void**arr, int l, int h, int (*compare)())
{
    assert(arr!=NULL && compare!=NULL);
    void*pivot = arr[l];
    int i = l;
    int j = h;

    while (i < j) {
        while (i <= h && compare(arr[i], pivot) <= 0) {
            i++;
        }
        while (j >= l && compare(arr[j], pivot) > 0) {
            j--;
        }
 ...
 * Complexity:  O(log n)
 *
 * Description:    recursively call partition for both sub arrays
 *		MAXELTS.
 */

static void quicksort(void**arr,int l, int h, int (*compare)())
{
    assert(arr!=NULL && compare!=NULL);
    int j;
    if(l<h)
    {
        j = partition(arr,l,h,compare);
        quicksort(arr,l,j,compare);
        quicksort(arr,j+1,h,compare);
    }
}
